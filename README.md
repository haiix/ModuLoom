...
- プレビューと依存物を確認できるサンプルギャラリー、複合ノード内部へドリルダウンできるブレークポイント対応DAG実行デバッガー
- 必須入力、型、循環、未使用ノード、コード生成可否を編集時に確認できるグラフ診断
- プロジェクトの IndexedDB 自動保存・復元と JSON バックアップ
...
まず試す場合は「その他の操作」の「サンプルギャラリー」から「四則演算パイプライン」または「配列データ変換パイプライン」を選ぶと、代表的な操作を確認できます。
...
評価処理、IndexedDBへの自動保存、JSONバックアップ、コード生成はブラウザ内で行われます。
現時点でバックエンドやサーバー側データベースはありません。
