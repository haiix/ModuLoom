...
import { NodeDefinition, NodeInstance, GraphEvaluation, TYPE_CONFIG, CustomTypeDefinition, getTypeStyle } from '../types';
import { ArrowRight, CheckCircle2, Clock, Play, Pause, RotateCcw, AlertTriangle } from 'lucide-react';
  ...
  if (hasCycle) {
    return (
      <div className="flex items-center gap-2 px-4 py-2.5 bg-red-500/10 border border-red-500/30 rounded-lg text-red-600 dark:text-red-400 text-xs">
        ...
        <span className="font-medium">循環参照（ループ）を検出しました: 有向非巡回グラフ (DAG) の制約に違反しているため評価を一時停止しています。</span>
      </div>
    );
  }
              ...
              >
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-slate-400">
                    ...
                    {evalResult?.isCached ? (
                      <span className="text-[9px] font-mono px-1 py-0.5 rounded bg-slate-200 dark:bg-slate-750 text-slate-500" title="前回の計算結果を保持">
                        cache
                      </span>
                    ) : evalResult?.durationMs !== undefined ? (
                      <span className="flex items-center gap-0.5 text-emerald-600 dark:text-emerald-400" title="差分再計算で実行">
