...
読み込み処理は `version` をマイグレーション表で解決してからスキーマ検証します。現行の
`1.1.0` を直接読み込めるほか、旧形式の `1.0.0` は `1.1.0` へ移行してから読み込みます。
それ以外の未対応バージョンは、現行バージョンを示すメッセージとともに拒否します。形式を
更新するときは、旧バージョンから現行バージョンへの変換をマイグレーション表へ追加します。
