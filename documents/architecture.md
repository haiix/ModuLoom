...
接続互換性は宣言型だけで判定し、実行値の変換は行いません。同型、片側が `any`、またはカスタム型から `object` への接続を許可します。`any` は双方向に互換なので、型安全性を弱めることに注意してください。責務、Validateの保証、ジェネリック型の採否と移行案は[型システム設計](./type-system.md)にまとめています。
...
- 機能的なグラフ編集や再評価では前の評価の `AbortController` を中断する

評価エンジンは1回の評価で同じ `AbortSignal` を各ノードと複合ノード内部へ渡します。組み込みPromiseはタイマーを解除し、Promise待機を中断します。Stream生成・変換・収集もSignalを監視し、停止時は `cancel` とIteratorの `return` をそれぞれ一度だけ呼びます。中断後の進捗と結果は通知せず、中断自体をノードエラーとして記録しません。自作式のWorkerも同じSignalで即時終了します。
...
各トレースには解決済み入力、出力、時間、キャッシュ利用、前回結果との差分、エラー伝播元からのノードID列を保存します。Promise待機中とStream収集中は待機状態を通知します。停止時はセッション共通の `AbortSignal` を中断し、対応するStreamの `cancel`／Iteratorの `return` をそれぞれ一度だけ呼びます。中断したトレースは `cancelled` として通常エラーから区別します。通常評価の結果は別に維持され、デバッグ画面を閉じると即座に通常表示へ戻ります。

`HierarchicalExecutionDebugger` はルートと各複合ノードに `DagExecutionDebugger` のフレームを作り、
複合ノードの外部入力をGroup Inputへ注入して内部サブグラフを同じstep/continue規則で実行します。
フレームは複合ノードIDの配列をパスとして持つため、ネストした複合ノードと同名の内部ノードを区別し、
ブレークポイントもパス付きキーで管理します。内部完了時はGroup Outputと外部出力のmappingから結果を
親フレームへ返し、内部エラーには階層パスを付けて親へ伝播します。すべての子フレームはルートの
AbortSignalへ接続され、編集または停止で子孫を逆順にキャンセルします。

UIスナップショットにはアクティブフレームのノード、パンくず、外部ポートと内部端子の対応を含めます。
完了済みフレームはセッション中保持されるため、親へ戻った後も内部トレースを再表示できます。
...
JSON保存時のドキュメント指紋と現在値を比較し、「JSON未書き出し」を判定します。これは
ブラウザ自動保存状態と分離します。`beforeunload` は自動保存の待機中・書込中・失敗中だけ
ページ離脱を警告し、保存完了後の再読み込みを妨げません。状態管理ライブラリは使っていません。
...
各タブは一時的な`writerId`を持ち、保存前にIndexedDBの同一readwriteトランザクションで
保存済みrevisionを比較します。他タブの更新を検出すると書き込みを中止し、現在タブのJSON
書き出しと保存済み状態の再読み込みを案内します。BroadcastChannelは早期通知にだけ使用し、
非対応環境でもrevision比較が競合を防ぎます。永続的なロックは使用しません。

保存先は責務ごとに分離します。

| 保存先        | 対象                                                         | 読み込みと障害時の扱い                                                                                                  |
| ------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| Local Storage | `moduloom:node-library-open`、`moduloom:onboarding-status`   | 起動時に同期読込が必要な軽量UI preference。利用不可ならviewport既定値または初回状態へフォールバックし、UI操作は継続する |
| IndexedDB     | `RecoverySnapshot`（プロジェクトとviewport、保存メタデータ） | 非同期で検証・復元する障害復旧状態。利用不可・読込失敗・書込失敗・容量超過は警告し、エディターを停止しない              |
| JSONファイル  | 明示的に書き出したプロジェクトとviewport                     | ブラウザ保存から独立したバックアップ・共有手段。同じ`parseFlowProject`で検証してから全体置換する                        |

2つのLocal Storageキーはプロジェクト非依存であり、JSONと`RecoverySnapshot`のどちらにも
含めません。不正または未対応の復元スナップショットも自動削除・部分適用せず、利用者が
画面上で明示的に破棄します。IndexedDBの復元でLocal Storageの設定は上書きしません。

自作式の信頼情報は `typeId` と `customCode` の正規化列に対するSHA-256としてスナップショット
へ保存します。起動時はプロジェクト検証後にfingerprintを再計算し、一致する場合だけエディター
をmountします。欠落・不正・不一致の場合は信頼確認画面で停止するため、同期評価、非同期評価、
